...
void route_detect_mode(int b_check, float x[], float y[], int m[], float aux_x[], float aux_y[], float fX[], float fY[], float vx[], float vy[], int ini_x[], int ini_y[], int min_fx[], int min_fy[], float final_route_x[], float final_route_y[])
{
	...
	vx[b_check] = fabs(fX[b_check]) * 0.004;
	vy[b_check] = fabs(fY[b_check]) * 0.004;
...
void att_ctloops(int b1, int b2, float ffxy[], float fX[], float fY[], float xyF[], int end_loop[], int loop_st[]){
   ...
   bn[1] = b2;
	...
	{
		aux = bn[fxy];
		float tfxy = fabs(fX[aux]) + fabs(fY[aux]);
		ffxy[aux] = tfxy;
	}

	for (int b_check = 0; b_check < 2; b_check++)
	{
		aux = bn[b_check];
		if (ffxy[aux] >= 0.5f)
		{
...
void trace_route(float x[], float y[], float fX[], float fY[], float vx[], float vy[], float ax[], float ay[], int num[], int nb_check, int vM[], int ini_xy[], int ini_mod[], int m[], float xyF[], int last_b[], int tgb[])
{
	//start variable run sys collision and dest route (coords)
	...
	float aux_x[16], aux_y[16], dst_target, dx, dy, dxloc, dyloc, dfdx, dfdy, calc_vy, calc_vx;
	...
	for (int b_check = 0; b_check < 16; b_check++){
		...
		if (ffxy[b_check] >= 0.5f){
			...
			total_percent_sum = total_percent_mf / 2;/// 2; //current percent mass force (x y)
	...
	int dec_all[0], ret_loop = 0, next_inc = 200;
	dec_all[0] = count_st;
    tgb[0] = 0;
	...
	while (dec_all[0] > 0){
		...
		for (int b_num_route = 0; b_num_route < 16; b_num_route++){
			...
			if(ffxy[b_num_route] >= 0.5f){
				loop_st[b_num_route] = loop_st[b_num_route] + 1;
	
				if (loop_st[b_num_route] > 249 && end_loop[b_num_route] > 0){
				//	end_loop[b_num_route] = 0;
					//dec_all[0] = dec_all[0] - 1;
				}
			}

			if (ffxy[b_num_route] >= 0.5f && end_loop[b_num_route] > 0 && loop_st[b_num_route] < next_inc && x[b_num_route] != 0 && y[b_num_route] != 0){
				next_route = true;
				...
				if (m[b_num_route] != 1 && m[b_num_route] != 10){
					...
					if (find_collision == false && end_loop[b_num_route] == 1){
				...
				if (find_collision == false && ffxy[b_num_route] >= 0.5f && end_loop[b_num_route] > 0 && x[b_num_route] != 0 && y[b_num_route] != 0){
		...
		for(int b_num_route = 0; b_num_route <= 15; b_num_route++){
	
			for (int b = 15; b >= 0; --b){

				if (x[b] != 0 && y[b] != 0 && num[b] != num[b_num_route] && ffxy[b_num_route] >= 0.5f && end_loop[b_num_route] > 0/* && find_collision == false*/){
					...
					if (find_collision == false && dst_target < 20){
	                    ...
	                    dxloc = aux_x[b_num_route] - aux_x[b];
						dyloc = aux_y[b_num_route] - aux_y[b];
							
						float dst_loc = sqrtf(dxloc*dxloc + dyloc*dyloc);
	                    ...
	                    /*Mix_Music *music = loadMusic("assets/libs/billiard_1c.mp3");					                                		
		        		Mix_PlayMusic(music, 0);*/
		        			
	                    route_detect_mode(b_num_route, x, y, m, aux_x, aux_y, fX, fY, vx, vy, ini_x, ini_y, min_fx, min_fy, final_route_x, final_route_y);                   								                 
	                     					                             					
	                  	route_detect_mode(b, x, y, m, aux_x, aux_y, fX, fY, vx, vy, ini_x, ini_y, min_fx, min_fy, final_route_x, final_route_y);
	                    att_ctloops(b_num_route, b, ffxy, fX, fY, xyF, end_loop, loop_st);
	                    /*	dec_all[0] = 0;
	                    for (int fxy = 0; fxy < 16; fxy++){
	                        float tfxy = fabs(fX[fxy]) + fabs(fY[fxy]);
	                           if(tfxy >= 1.35f && end_loop[fxy]  == 1){  
	                            dec_all[0] = dec_all[0] + 1;
	                           }
	                     }*/							
					}
...
void engine_show_all(SDL_Renderer *rendEngine, int n[], int m[], float vx[], float vy[], float x[], float y[], float ax[], float ay[], float deg[], int key_all[], int ini_xy[], char mode_p[], int ex[], int ey[], int ini_mod[], float vFX[], float vFY[], int vM[], float xyF[], int lastProxTarget[], int last_b[])
{
	...
	if (lastProxTarget[0] == 100)
	{
		...
		trace_route(x, y, vFX, vFY, vx, vy, ax, ay, n, 0, vM, ini_xy, ini_mod, m, xyF, last_b, lastProxTarget);
	}
	...
	while (run == true)
	{
		...
		//	check_table(v_key, m, x, y, vFX, vFY, ini_xy, n, ini_mod);
			...
			{
				deg[v_key] = 25 * vFX[v_key]  * (vFX[v_key] * 180 * vx[v_key]);
			}
			else
			{
				deg[v_key] = 25 * vFY[v_key] * (vFY[v_key] * 180 * vy[v_key]);
			}
